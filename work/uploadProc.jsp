...
String file1 = "", file2 = ""; // 업로드한 파일명을 저장할 변수(업로드시 이름이 바뀔 수도 있음)
...
try {
	MultipartRequest multi = new MultipartRequest(
			...
			uploadPath, // 서버에 실제로 파일이 저장도리 위치 지정
			maxSize, // 한 번에 업로드할 수 있는 최대크기(byte단위)
			"utf-8", // 파일의 인코딩 방식(utf-8, euc-ke, ksc-5601 등)
			new DefaultFileRenamePolicy()); // 파일 이름의 중복 처리
	
	name = multi.getParameter("name");
	title = multi.getParameter("title");
	// enctype을 'multipart/form-data'로 지정하면 request객체로는 데이터를 받을 수 없고 반드시 업로드 객체로 받아야 함

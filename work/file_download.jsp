...
// '\' 하나는 뒤의 한글자를 그대로 출력하는 용도로 사용되기 때문 '\'를 출력하려면 두번입력해야 함
...
if (ie) {
	...
	// 파일명에 띄어쓰기가 있을 경우 IE는 '+'로 변경하나, 파일 다운로드 '+'를 사용할 순 없으므로 다시 띄어쓰기로 변환하는 작업
} else {
...
out2.flush(); // out2에 남아 있는 데이터가 있으며 모두 다운로드 시키라는 의미
out2.close(); // out2를 닫음
in.close(); // in을 닫음
%>